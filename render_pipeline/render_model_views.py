...
for param in view_params:
    ...
    for i in range(random.randint(light_num_lowbound, light_num_highbound)):
        ...
        bpy.ops.object.lamp_add(type='HEMI', view_align = False, location=(lx, ly, lz))
        tmp_energy = np.random.normal(g_syn_light_energy_mean, g_syn_light_energy_std)
        if tmp_energy < 0:
            tmp_energy = 0.4
        elif tmp_energy < 0.1:
            tmp_energy += 0.4
        bpy.data.objects['Hemi'].data.energy = tmp_energy
    ...
    syn_image_file = './%s_%s_a%03d_e%03d.png' % (shape_synset, shape_md5, round(azimuth_deg), round(elevation_deg))
